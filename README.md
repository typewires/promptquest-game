...
2. Goal (free, sampled)
Each level gets one or more goal types (stacked) from: `cure`, `key_and_door`, `lost_item`, `repair_bridge`. You can select stacks per level via the UI or prompt directives.
3. Story + Sprites (AI)
...
| Goals | Code (sampled from a fixed list) | You can force per-level goals (UI or `Level N:` lines) |
...
You can also stack multiple goals by listing more than one, for example:
- `Level 1: cure, repair_bridge`
- `Level 2: key_and_door + lost_item`
